...
public class BoardUI {
    ...
    private static final Color gridColor = Library.color(64);
    ...
    private static final Color infoPanelColor = Library.color(192);
    private static final Color infoPanelNameColor = Library.color(224);
    private static final Color fontColor = Library.color(16);
    private static final Color infoPanelStickColor = Library.color(160);
    ...
    public BoardUI() {
        ...
        try {
            font = Font
                    .createFont(Font.TRUETYPE_FONT, getClass().getResourceAsStream(("/quoridor/resource/consola.ttf")))
                    .deriveFont(20f);
    ...
    private void initPanel() {
        ...
        panel.setBackground(Library.color(255));
        panel.setPreferredSize(new Dimension(canvasWidth, canvasHeight));
        panel.addMouseMotionListener(new MouseAdapter() {
            @Override
            public void mouseMoved(MouseEvent e) {
                boolean isHover = checkHover(e.getX(), e.getY(), false);
                panel.setCursor(new Cursor(isHover ? Cursor.HAND_CURSOR : Cursor.DEFAULT_CURSOR));
            }
        });
    ...
    private void setStickOffset() {
        stickCountOffset[0] =
                (infoPanelWidth - Library.getTextWidth(font, String.valueOf(bm.getWallCount(0)))) / 2 + infoPanelRx;
        stickCountOffset[1] =
                (infoPanelWidth - Library.getTextWidth(font, String.valueOf(bm.getWallCount(1)))) / 2 + paddingWidth;
    }

    private void moveWall(int x, int y) {
        if (wall != null) {
            wall.setLocation(x - (wall.isRotated() ? wallHalfLength : paddingHalfWidth),
                    y - (wall.isRotated() ? paddingHalfWidth : wallHalfLength));
        }
    }
    ...
    private void refreshInputList() {
        ...
        if (!wallMode) {
            ...
            for (Point p : movableInfo) {
                inputList.add(new MouseCheck(boardOffset.x + p.x * (gridSize + paddingWidth) + paddingWidth,
                        boardOffset.y + p.y * (gridSize + paddingWidth) + paddingWidth, gridSize, gridSize, () -> {
            ...
            if (bm.getWallCount(bm.getPlayerNow()) > 0) {
                ...
                inputList.add(new MouseCheck(bm.getPlayerNow() == 0 ? infoPanelWallRx : infoPanelWallLx,
                        bm.getPlayerNow() == 0 ? infoPanelWallRy : infoPanelWallLy, paddingWidth, wallLength, cbWall));
            }
        } else {
            for (int y = 0; y < gridCount - 1; y++) {
                for (int x = 0; x < gridCount - 1; x++) {
                    int[][] puttable = bm.getPuttableWall();
                    if ((puttable[x][y] & (wall.isRotated() ? 2 : 1)) > 0) {
                        final int dx = x;
                        final int dy = y;
                        inputList.add(new MouseCheck(
                                boardOffset.x + (gridSize + paddingWidth) * (x + 1) - paddingHalfWidth + circleRadius,
                                boardOffset.y + (gridSize + paddingWidth) * (y + 1) - paddingHalfWidth + circleRadius,
                                circleRadius, () -> {
                                    bm.putWall(dx, dy, wall.isRotated() ? 1 : 2, 0, null);
                                    wallMode = false;
                                    panel.remove(wall);
                                    setStickOffset();
                                    refreshInputList();
    ...
    private void drawBoard(Graphics2D g) {
        ...
        for (int y = 0; y < BoardModel.gridCount; y++) {
            for (int x = 0; x < BoardModel.gridCount; x++) {
                ...
                g.fillRect(x * (gridSize + paddingWidth) + paddingWidth, y * (gridSize + paddingWidth) + paddingWidth,
                        gridSize, gridSize);
                g.setColor(gridColor);
            }
        }
        ...
        for (int y = 0; y < gridCount - 1; y++) {
            for (int x = 0; x < gridCount - 1; x++) {
                int[][] wall = bm.getWall();
                if (wall[x][y] == 1) {
                    g.fillRect((x + 1) * (gridSize + paddingWidth) - gridSize, (y + 1) * (gridSize + paddingWidth),
                            wallLength, paddingWidth);
                }
                if (wall[x][y] == 2) {
                    g.fillRect((x + 1) * (gridSize + paddingWidth), (y + 1) * (gridSize + paddingWidth) - gridSize,
                            paddingWidth, wallLength);
                }
            }
        }
        ...
        if (wallMode) {
            ...
            for (int y = 0; y < gridCount - 1; y++) {
                for (int x = 0; x < gridCount - 1; x++) {
                    if ((puttable[x][y] & (wall.isRotated() ? 2 : 1)) > 0) {
                        g.fillOval((gridSize + paddingWidth) * (x + 1) - paddingHalfWidth,
                                (gridSize + paddingWidth) * (y + 1) - paddingHalfWidth, circleDiameter, circleDiameter);
                    }
                }
            }
        }
